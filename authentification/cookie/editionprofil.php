...
if(isset($_COOKIE['id'])) {
   ...
   if(isset($_POST['newprenom']) AND !empty($_POST['newprenom']) AND $_POST['newprenom'] != $user['prenom']) {
    ...
    $insertprenom->execute(array($newprenom, $_COOKIE['id']));
    header('Location: profil.php?id='.$_COOKIE['id']);
 }
   if(isset($_POST['newmail']) AND !empty($_POST['newmail']) AND $_POST['newmail'] != $user['mail']) {
      $newmail = htmlspecialchars($_POST['newmail']);
      $insertmail = $bdd->prepare("UPDATE membres SET mail = ? WHERE id = ?");
      $insertmail->execute(array($newmail, $_COOKIE['id']));
      header('Location: profil.php?id='.$_COOKIE['id']);
   }
   if(isset($_POST['newmdp1']) AND !empty($_POST['newmdp1']) AND isset($_POST['newmdp2']) AND !empty($_POST['newmdp2'])) {
      ...
      if($mdp1 == $mdp2) {
         ...
         $insertmdp->execute(array($mdp1, $_COOKIE['id']));
         header('Location: profil.php?id='.$_COOKIE['id']);
      } else {
   ...
   if(isset($_FILES['avatar']) AND !empty($_FILES['avatar']['name'])) {
      ...
      if($_FILES['avatar']['size'] <= $tailleMax) {
         ...
         if(in_array($extensionUpload, $extensionsValides)) {
            $chemin = "../img/Avatar/".$_COOKIE['id'].".".$extensionUpload;
            $resultat = move_uploaded_file($_FILES['avatar']['tmp_name'], $chemin);
            if($resultat) {
               $updateavatar = $bdd->prepare('UPDATE membres SET avatar = :avatar WHERE id = :id');
               $updateavatar->execute(array(
                  'avatar' => $_COOKIE['id'].".".$extensionUpload,
                  'id' => $_COOKIE['id']
                  ));
               header('Location: profil.php?id='.$_COOKIE['id']);
            } else {
